...
class SessionEntry:
    ...
    def to_public_payload(self, *, include_token: bool = False) -> dict[str, Any]:
        payload = {
            "label": self.label,
            "schema": self.schema,
...
class SessionStore:
    ...
    def _purge_expired_locked(self, now: datetime) -> None:
        expired_tokens = [token for token, entry in self._sessions.items() if entry.expires_at <= now]
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        expose_session_tokens=_env_bool("SQL_MCP_EXPOSE_SESSION_TOKENS", False),
    )
...
def _ensure_driver(database_url: str) -> None:
    ...
    try:
        ...
        if not pkg:
            raise RuntimeError(
                f"Missing database driver for scheme '{scheme}', and no auto-install package mapping is defined. Install the required driver manually before starting the server."
            ) from exc
        if not SETTINGS.auto_install_drivers:
            raise RuntimeError(
                f"Missing database driver for scheme '{scheme}'. Install '{pkg}' manually or enable SQL_MCP_AUTO_INSTALL_DRIVERS=true to allow runtime installation."
            ) from exc
        ...
        try:
            ...
            raise RuntimeError(
                f"Driver installation for scheme '{scheme}' did not make the driver importable. Install '{pkg}' manually and retry."
            ) from retry_exc
...
def _connect(context: RequestContext):
    return _get_engine(context).connect()
...
def _resolve_request_context(
    ...
    resolved_schema = (schema or (session_entry.schema if session_entry else None) or SETTINGS.default_schema).strip()
    if not resolved_schema:
        raise ValueError("schema must be non-empty.")
    resolved_timeout = statement_timeout_ms or (session_entry.statement_timeout_ms if session_entry else None) or SETTINGS.statement_timeout_ms
    if resolved_timeout <= 0:
        raise ValueError("statement_timeout_ms must be greater than zero.")
    resolved_max_rows = max_rows or (session_entry.max_rows if session_entry else None) or SETTINGS.max_rows
...
def _require_read_permission(permissions: dict[str, bool]) -> None:
    if not permissions.get("allow_read"):
        raise ValueError("Metadata access rejected: 'allow_read' permission is required.")
...
def _validate_database_connection(context: RequestContext) -> dict[str, Any]:
    ...
    with _connect(context) as conn:
        ...
        if "postgres" in engine.name:
            ...
            if res:
                database_name, current_user = res
    return {"database_name": database_name, "current_user": current_user}
...
def create_session(
    ...
    return {
        "ok": True,
        "session": session.to_public_payload(include_token=True),
...
@mcp.tool()
@_safe_tool
def list_sessions(include_tokens: bool = False) -> dict[str, Any]:
    expose_tokens = include_tokens and SETTINGS.expose_session_tokens
    sessions = [entry.to_public_payload(include_token=expose_tokens) for entry in SESSION_STORE.list()]
    return {"count": len(sessions), "tokens_exposed": expose_tokens, "sessions": sessions}
...
def list_config_databases() -> dict[str, Any]:
    ...
    if not os.path.exists(config_path):
        return {"ok": False, "error": "mcp-config.json not found in current directory.", "cwd": os.getcwd()}
    try:
        ...
        sanitized = []
        for db in databases:
            sanitized.append({"name": db.get("name"), "description": db.get("description"), "has_url": bool(db.get("database_url"))})
...
def connect_to_config_database(
    ...
    return create_session(
        ...
        label=label or name,
        allow_read=target.get("allow_read", allow_read),
        allow_insert=target.get("allow_insert", allow_insert),
        allow_update=target.get("allow_update", allow_update),
        allow_delete=target.get("allow_delete", allow_delete),
        allow_create=target.get("allow_create", allow_create),
        allow_drop=target.get("allow_drop", allow_drop),
    )


@mcp.tool()
@_safe_tool
def get_session_info(session_token: str, include_token: bool = False) -> dict[str, Any]:
    session = SESSION_STORE.get(session_token)
    expose_token = include_token and SETTINGS.expose_session_tokens
    return {"ok": True, "token_exposed": expose_token, "session": session.to_public_payload(include_token=expose_token)}


@mcp.tool()
@_safe_tool
def list_tables(schema: str | None = None, session_token: str | None = None) -> dict[str, Any]:
    context = _resolve_request_context(session_token=session_token, schema=schema)
    _require_read_permission(context.permissions)
    with _connect(context) as conn:
        _apply_connection_constraints(conn, context)
...
@mcp.tool()
@_safe_tool
def list_views(schema: str | None = None, session_token: str | None = None) -> dict[str, Any]:
    context = _resolve_request_context(session_token=session_token, schema=schema)
    _require_read_permission(context.permissions)
    with _connect(context) as conn:
        _apply_connection_constraints(conn, context)
...
@mcp.tool()
@_safe_tool
def list_functions(schema: str | None = None, session_token: str | None = None) -> dict[str, Any]:
    context = _resolve_request_context(session_token=session_token, schema=schema)
    _require_read_permission(context.permissions)
    sql = """
        select routine_schema, routine_name, specific_name, data_type
...
@mcp.tool()
@_safe_tool
def list_referenced_tables(table: str, schema: str | None = None, session_token: str | None = None) -> dict[str, Any]:
    context = _resolve_request_context(session_token=session_token, schema=schema)
    _require_read_permission(context.permissions)
    ...
    return {"schema": schema_name, "table": table_name, "count": len(rows), "referenced_tables": rows}


@mcp.tool()
@_safe_tool
def list_referencing_tables(table: str, schema: str | None = None, session_token: str | None = None) -> dict[str, Any]:
    context = _resolve_request_context(session_token=session_token, schema=schema)
    _require_read_permission(context.permissions)
    schema_name, table_name = _qualified_name(context.schema, table)
    rows = []
    try:
        ...
        pass
    return {"schema": schema_name, "table": table_name, "count": len(rows), "referencing_tables": rows}


@mcp.tool()
@_safe_tool
def list_related_tables(table: str, schema: str | None = None, session_token: str | None = None) -> dict[str, Any]:
    context = _resolve_request_context(session_token=session_token, schema=schema)
    _require_read_permission(context.permissions)
    referenced = list_referenced_tables(table, context.schema, session_token)
    referencing = list_referencing_tables(table, context.schema, session_token)
...
@mcp.tool()
@_safe_tool
def list_related_tables_detailed(table: str, schema: str | None = None, session_token: str | None = None) -> dict[str, Any]:
    context = _resolve_request_context(session_token=session_token, schema=schema)
    _require_read_permission(context.permissions)
    referenced = list_referenced_tables(table, context.schema, session_token)
    referencing = list_referencing_tables(table, context.schema, session_token)
...
@mcp.tool()
@_safe_tool
def describe_table(table: str, schema: str | None = None, session_token: str | None = None) -> dict[str, Any]:
    context = _resolve_request_context(session_token=session_token, schema=schema)
    _require_read_permission(context.permissions)
    schema_name, table_name = _qualified_name(context.schema, table)
    try:
        ...
        rows = []
    return {"schema": schema_name, "table": table_name, "count": len(rows), "columns": rows}
    ...
    with _connect(context) as conn:
        with conn.begin():
            ...
            if result.returns_rows:
                rows = [tuple(r) for r in result.fetchmany(row_limit)]
    return {
...
def main() -> None:
    ...
    if transport == "both":
        ...
        http_thread = threading.Thread(target=mcp.run, kwargs={"transport": "streamable-http"}, daemon=True)
        http_thread.start()
        try:
